...
function hotelList(hotel) {
  ...
  function tabList(city) {
    return (
      <div class='mt-12'>
      <button className={activeButton === city.city?'active' : 'normal'} onClick={()=>{handleButtonClick(city.city);setN(!n);
    setCityName(city.city);}} id={city.city}>{city.city}</button>
    </div>
    );
  }
  function showMore(){
  ...
  function showLess(){
    ...
    const para ='Real estate can be brought, sold, leased, or rented, and can be a valuable investment opportunity. The value of real estate can be...'
    return(
        <>
        ...
        <div class='w-1/6 m-auto'>
            <button class='bg-dark-blue px-6 py-2 mb-10 flex justify-center items-center ml-4 text-white font-semibold rounded-3xl' id='showMoreBtn' onClick={showMore}><img src={hourglass} class='mr-1'></img>Show More</button>
        </div>
        <div class='w-1/6 m-auto'>
            <button class='bg-dark-blue px-6 py-2 mb-10 ml-8 text-white font-semibold rounded-3xl hidden' id='showLessBtn' onClick={showLess}>Show Less</button>
        </div>
        </>
    )
}
